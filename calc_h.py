...
def calc_h (gamma, aoa, q, P_inf, C_pmax, M_inf, T_localstag, T_wall, x, Re, Pr, vel, isLaminar):
    # Calculating T_local
    P_local = q*C_pmax*((m.cos(aoa))**2) + P_inf                              # Eq 11
    ...
    M_local = m.sqrt((((P_localstag/P_local)**((gamma-1)/gamma))-1)*(2/(gamma-1)))        # Eq 7
    T_local = T_stag/(1+((gamma-1)/2)*(M_local**2))                      # Eq 13
